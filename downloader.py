...
URL = "https://minecraft.curseforge.com/projects/{0}"
DEPENDENCY_URL = "https://minecraft.curseforge.com/projects/{0}/relations/dependencies?filter-related-dependencies=3"
NO_VERSION = "No file found for mod {1} with minecraft version {0}"

FILE_ID = re.compile(r"(?P<file_id>\d+)/download")


def find_file_id(mc_version: str, mod_name: str, soup: BeautifulSoup) -> int:
    # print(soup.find_all('a'))
    elements = [item for item in soup.find_all('a') if mc_version == item.text.strip()]

    if not elements:
        pass  # raise Exception(NO_VERSION.format(mc_version, mod_name))
...
if __name__ == "__main__":
    result = download("Minecraft 1.12", "Railcraft")
    print(result)
